...
export default function PsychologyAssessment({ patient, onSubmit, onBack, mode }) {
  ...
  // ── Mode-based delegation (after all hooks) ──
  if (mode === "followup") {
    return (
  ...
  if (mode === "progress" || mode === "group") {
    return (
      <div style={{ padding: 48, textAlign: "center", background: "#F5F7FA", minHeight: "100vh" }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>🚧</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#374151", marginBottom: 6 }}>
          {mode === "progress" ? "Progress Intervention" : "Group Intervention"} — Coming Soon
        </div>
        <div style={{ fontSize: 14, color: "#6B7280", marginBottom: 24 }}>
          This module is under development for Psychology.
        </div>
        <button
          onClick={onBack}
  ...
  // ── Initial assessment (mode === "initial" or undefined) ──
  const problemList = (vals) => {
    let text = '';
    if (vals.perceptual_disturbance) {
      vals.perceptual_disturbance.forEach(v => {
        text += v.charAt(0).toUpperCase() + v.slice(1).replaceAll('_', ' ') + '\n';
      });
    }
    if (vals.thought_content_patient_reported) {
      vals.thought_content_patient_reported.forEach(v => {
        text += v.charAt(0).toUpperCase() + v.slice(1).replaceAll('_', ' ') + '\n';
      });
    }
    return text;
          ...
          if (field.type === "radio-group") {
            ...
            return (
              <div key={`${si}-${fi}`} style={{
                ...
                {/* Radio options — right-aligned, wrap if needed */}
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 16px", flex: 1, justifyContent: "flex-end" }}>
