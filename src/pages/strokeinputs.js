...
const afterFistSubmit = () => {
  ...
  for (const key in inputValues) { // Correctly using `for...in` loop
          ...
          newInputValues[key] = value + 2; // Increment the value by 2
          setVideoSource("/Normal.mp4")
      setIsModifiedBBG("Modified BBG");
      setIsModifiedFist("Modified Fist");
  setInputValues(newInputValues);
          }}
};
const afterMasSubmit = () => {
  const newInputValues = {};
  for (const key in inputValues) { // Correctly using `for...in` loop
          const value = parseFloat(inputValues[key]) || 0;
           if(value == ''){
            ...
            newInputValues[key] = value + 2;
            setVideoSource("/Normal.mp4")
            setIsModifiedMas("Modified Mas");
  setInputValues(newInputValues); // Increment the value by 2
           }
...
const handleShowInputs = (disease) => {
  ...
  for (const key in inputValues) { // Correctly using `for...in` loop
          const value = parseFloat(inputValues[key]) || 0; // Ensure the value is a number, defaulting to 0 if not
          newInputValues[key] = ''; // Increment the value by 2
      }
  setIsModified("MMT")
  setIsModifiedBBG("BBG")
  setIsModifiedFist("Fist")
  setIsModifiedMas("Mas")
  setVideoSource("/60per.mp4")
  setInputValues(newInputValues);
...
<div className="center">
  ...
  <b><label htmlFor="cType">Types: </label></b>
   ...
   <b><label htmlFor="tType">Level:</label></b>
      ...
      <select id="disease" style={{ height: '65px' }}multiple value={selectedDiseases} onChange={handleDiseaseChange} disabled={!(cType && tType)}>
...
{showInputsFor === 'Strength' && (
         ...
         <button id = "MMT"onClick={handleClick}>{isModified}</button>
         {showMMTInputs && (
...
<td  colSpan={6} style={{textAlign:'center', justifyContent: 'center',marginTop:'10px'}}> 
    ...
    <input style={{width:'30%',marginLeft:'10px'}} id="DaysForTreatment" name="Days_Of_Treatment" value={inputValues.Days_Of_Treatment} onChange={handleChange} type="number" />
        ...
        <div>
        <button className="B1" onClick={afterStrokeSubmit}>Submit Data</button>
...
{showInputsFor === 'Balance' && (
  <div style={{ marginBottom: "20px", padding: "10px", marginTop: "20px" }}>
    <button onClick={handleClick2}>{isModifiedBBG}</button>
    <button onClick={handleClick3}>{isModifiedFist}</button>
    {showBBGInputs && (
            <div id="input-container"style={{ marginBottom: "20px", padding: "10px", marginTop: "20px" }} >
                 <table style={{ width: "100%" }}>
                        <tbody>
...
<td colSpan={3} style={{ justifyContent: 'center',marginTop:'10px'}}>
<label htmlFor="Initial_Deviation_From_Ideal_Single_Support_Right">Berg Balance Scale</label>
<input style={{width:'20%',marginLeft:'10px'}}id="Initial_Deviation_From_Ideal_Single_Support_Right" name="Left_Fingers_Abduction" value={inputValues.Left_Fingers_Abduction} onChange={handleChange} type="number" />
</td>
...
<tr>
  <td  colSpan={6} style={{textAlign:'center', justifyContent: 'center',marginTop:'10px'}}> 
    ...
    <input style={{width:'30%',marginLeft:'10px'}} id="DaysForTreatment" name="Days_Of_Treatment" value={inputValues.Days_Of_Treatment} onChange={handleChange} type="number" />
        ...
        <div>
        <button className="B1" onClick={afterFistSubmit}>Submit Data</button>
      <button style={{float:'right'}} onClick={openModal}>Animate</button>
...
{showInputsFor === 'Spasticity' && (
         <div style={{ marginBottom: "20px", padding: "10px", marginTop: "20px" }}>
         <button onClick={handleClick4}>{isModifiedMas}</button>
         {showMASInputs && (
            <div id="input-container"style={{ marginBottom: "20px", padding: "10px", marginTop: "20px" }} >
  <table style={{ width: "100%" }}>
              <tbody>
